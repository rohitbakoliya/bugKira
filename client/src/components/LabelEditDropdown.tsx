...
const LabelEditDropdown: React.FC<LabelDropdownProps> = ({
  ...
  }, [checkedItems, updateSelectedLabels]);
    ...
    <StyledLabelEditDropdown>
      <Dropdown>
        ...
        <Dropdown.Content className={'label__dropdown--content'}>
          {checkboxes.map(name => {
            let selectedClass = checkedItems.includes(name) ? 'label__selected' : '';
          ...
          <Dropdown.Toggle>{(_toggle, close) => children(close)}</Dropdown.Toggle>
        </Dropdown.Content>
      </Dropdown>
    </StyledLabelEditDropdown>
  );
