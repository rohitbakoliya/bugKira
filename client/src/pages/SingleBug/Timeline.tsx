...
const Timeline: React.FC<TimelineProps> = ({ author, date, from, action }) => {
  ...
  let renderExcerpt = (
    <span>
      {!isReference && <>{action === 'opened' ? 'Opened' : 'Closed'} by&nbsp;</>}
  ...
  return (
    <StyledTimeline>
      <Flex gap="large" align="center" nowrap>
        <CircleIcon className="icon" size="30px" variant={variant} icon={icon} />

        <Avatar width="30px" height="30px" size={45} username={author.username} />
